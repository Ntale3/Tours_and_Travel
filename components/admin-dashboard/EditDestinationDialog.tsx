...
import { updateDestination, fetchDestinations } from '@/store/destinations.store';
...
// Edit Destination Dialog Component
interface EditDestinationDialogProps {
  destination:Destination
  trigger?: React.ReactNode;
}

export function EditDestinationDialog({ destination, trigger }: EditDestinationDialogProps) {
  ...
  const [formData, setFormData] = useState<DestinationFormData>({
    ...
    booking_fee: destination.booking_fee?.toString() || '',
  ...
  const [amenities, setAmenities] = useState<string[]>(destination.amenities || []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateDestination(destination.id, formDataToSend);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl">Edit Destination</DialogTitle>
          <DialogDescription>
            Update the destination details below. All fields marked with * are required.
          </DialogDescription>
        </DialogHeader>
